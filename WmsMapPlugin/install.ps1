...
if (-not $SkipAutoCAD) {
  ...
  if (-not (Test-Path $autodeskDir)) {
    ...
    Write-Host "  警告: 找不到 AutoCAD 插件包: $sourceBundle" -ForegroundColor Yellow
    ...
    if (-not (Test-Path $dllPath)) {
      Write-Host "  警告: 找不到编译产物: $dllPath" -ForegroundColor Yellow
...
if (-not $SkipWPS) {
  ...
  if (-not (Test-Path $wpsDir)) {
    ...
    Write-Host "  警告: 找不到 WPS 插件目录: $wpsSource" -ForegroundColor Yellow
  } else {
    # 创建目标目录
